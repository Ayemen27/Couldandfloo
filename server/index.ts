  ...
  try {
    ...
    if (dbCheck.success) {
      ...
      if (testResult.success) {
        ...
        // تحسين نظام الإكمال التلقائي - مؤقتاً معطل
        try {
          log("🔧 نظام الإكمال التلقائي جاهز للتفعيل");
          log("✅ تم إعداد نظام الإكمال التلقائي بنجاح");
        } catch (error) {
        ...
        // تسجيل جميع المسارات من ملف routes.ts
        try {
          log("📋 بدء تسجيل جميع مسارات API...");
          await registerRoutes(app);
          log("✅ تم تسجيل جميع مسارات API بنجاح");
        } catch (error) {
          log("❌ خطأ في تسجيل المسارات:");
          console.error(error);
        }
      } else {
